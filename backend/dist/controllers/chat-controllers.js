...
export const chatGuidelines = async (req, res, next) => {
    try {
...
export const addGuidelines = async (req, res, next) => {
...
export const editGuidelines = async (req, res, next) => {
    try {
        const { title, description } = req.body;
        if (!title || !description) {
            return res.status(400).json({ message: 'Title and description are required' });
        }
        const updatedGuideline = await Guidelines.findByIdAndUpdate(req.params.id, { title, description }, { new: true });
...
export const deleteGuidelines = async (req, res, next) => {
    try {
